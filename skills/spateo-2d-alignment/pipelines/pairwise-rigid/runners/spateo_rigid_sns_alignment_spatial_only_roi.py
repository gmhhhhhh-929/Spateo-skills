...
SIGMA2_INIT_SCALE = float(os.environ.get("SIGMA2_INIT_SCALE", "2"))
SIGMA2_END_RAW = os.environ.get("SIGMA2_END", "").strip()
SIGMA2_END = float(SIGMA2_END_RAW) if SIGMA2_END_RAW else None
RUN_LABEL = os.environ.get("RUN_LABEL", "").strip()
EXPRESSION_MODE = os.environ.get("EXPRESSION_MODE", "normal").strip().lower()
DUMMY_REP_DIM = int(os.environ.get("DUMMY_REP_DIM", "30"))
ALIGNMENT_ROI_JSON = os.environ.get("ALIGNMENT_ROI_JSON", "").strip()
...
if EXPRESSION_MODE not in {"normal", "spatial_only"}:
    raise SystemExit("EXPRESSION_MODE must be 'normal' or 'spatial_only'.")
if DUMMY_REP_DIM <= 0:
    raise SystemExit("DUMMY_REP_DIM must be positive.")


def parse_alignment_roi_rules() -> list[dict[str, Any]]:
    if not ALIGNMENT_ROI_JSON:
        return []
    try:
        rules = json.loads(ALIGNMENT_ROI_JSON)
    except json.JSONDecodeError as exc:
        raise SystemExit(f"ALIGNMENT_ROI_JSON is not valid JSON: {exc}") from exc
    if isinstance(rules, dict):
        rules = [rules]
    if not isinstance(rules, list):
        raise SystemExit("ALIGNMENT_ROI_JSON must be a JSON object or list of objects.")
    for rule in rules:
        if not isinstance(rule, dict):
            raise SystemExit("Each ALIGNMENT_ROI_JSON rule must be an object.")
    return rules


ALIGNMENT_ROI_RULES = parse_alignment_roi_rules()
...
def roi_rule_matches_slice(rule: dict[str, Any], row: pd.Series) -> bool:
    labels = rule.get("slice_labels") or []
    if "sl_number" in rule:
        labels = list(labels) + [rule["sl_number"]]
    sl_number = int(row["sl_number"])
    slice_id = str(row["slice_id"])
    if labels:
        wanted = {norm_slice_token(x) for x in labels}
        return norm_slice_token(slice_id) in wanted or str(sl_number) in wanted
    start = rule.get("slice_start")
    end = rule.get("slice_end")
...
def apply_alignment_roi_filters(
    row: pd.Series,
    spatial: np.ndarray,
    candidate_idx: np.ndarray,
) -> tuple[np.ndarray, list[dict[str, Any]]]:
    summaries = []
    for rule_index, rule in enumerate(ALIGNMENT_ROI_RULES, start=1):
        if not roi_rule_matches_slice(rule, row):
            continue
        xy = spatial[candidate_idx]
        ...
        for axis_name, axis_idx in [("x", 0), ("y", 1)]:
            for op_name, op_fn in [
                ...
                key = f"keep_{axis_name}_{op_name}"
                if key in rule:
                    value = float(rule[key])
                    keep &= op_fn(xy[:, axis_idx], value)
                    conditions.append({"key": key, "value": value})
        before = int(len(candidate_idx))
        ...
        summaries.append(
            {
                ...
                "before": before,
                "after": int(len(candidate_idx)),
                "excluded": int(before - len(candidate_idx)),
            }
        )
    return candidate_idx, summaries
...
def load_slice(row: pd.Series, rng: np.random.Generator) -> ad.AnnData:
    with h5py.File(row["file"], "r") as handle:
        ...
        if EXCLUDE_CELLTYPES_REGEX and "celltype" in handle["obs"]:
            ...
            candidate_idx = candidate_idx[keep]
        candidate_idx, roi_summaries = apply_alignment_roi_filters(row, spatial, candidate_idx)

        n_candidates = len(candidate_idx)
        if n_candidates == 0:
            raise SystemExit(f"Alignment ROI removed all cells from {row['slice_id']}.")
        ...
        if EXPRESSION_MODE == "spatial_only":
            adata.obsm["X_pca"] = np.ones((len(idx), DUMMY_REP_DIM), dtype=np.float32)
        else:
            adata.obsm["X_pca"] = x_pca[idx].astype(np.float32)
        adata.uns["alignment_roi_filter"] = roi_summaries
        return adata
...
def run_morpho_stage(
    ...
    common_kwargs = dict(
        ...
        sigma2_init_scale=SIGMA2_INIT_SCALE,
    )
    if SIGMA2_END is not None:
        common_kwargs["sigma2_end"] = SIGMA2_END
...
def main() -> None:
    ...
    print("expression_mode", EXPRESSION_MODE, flush=True)
    print("dummy_rep_dim", DUMMY_REP_DIM, flush=True)
    print("sigma2_init_scale", SIGMA2_INIT_SCALE, flush=True)
    print("sigma2_end", SIGMA2_END, flush=True)
    print("alignment_roi_rules", json.dumps(to_jsonable(ALIGNMENT_ROI_RULES)), flush=True)
    ...
    provenance = {
        ...
        "dummy_rep_dim": DUMMY_REP_DIM,
        "alignment_roi_json": ALIGNMENT_ROI_JSON,
        "alignment_roi_rules": ALIGNMENT_ROI_RULES,
        "sigma2_init_scale": SIGMA2_INIT_SCALE,
    ...
    for adata in raw_slices:
        print(
            ...
            "alignment_roi_filter",
            json.dumps(to_jsonable(adata.uns.get("alignment_roi_filter", []))),
            flush=True,
        )
