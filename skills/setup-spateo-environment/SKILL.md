...
This is stage 1 of the ordered collection: environment → IO → slice quality with viewer → 2D alignment → 3D pipeline → native 4D pipeline. The current 3D implementation covers point-cloud VTK reconstruction; later 3D models remain pending. The IO, 3D point-cloud, and 4D skills specify their verified source revision; use that revision when preparing their environment. Check each downstream skill's input/runtime contracts. Environment smoke tests do not establish full pipeline execution or biological accuracy.
