...
This is stage 1 of the ordered collection: environment → IO → slice quality with viewer → 2D alignment → reserved 3D pipeline → native 4D pipeline. The IO and 4D skills specify their newer verified source revision; use that revision when preparing their environment. Check each downstream skill's input/runtime contracts. Environment smoke tests do not establish full pipeline execution or biological accuracy.
